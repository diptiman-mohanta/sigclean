...
# Add the repo root (parent of this examples/ directory) to the path, so this
# script works even without an installed/editable sigclean package.
...
def main():
    ...
    csv_file = args.csv
    if not csv_file:
        # Look for common CSV files alongside this script (examples/), not
        # just the current working directory, since this script is usually
        # invoked from elsewhere (e.g. the repo root).
        script_dir = os.path.dirname(os.path.abspath(__file__))
        possible_files = [
            os.path.join(script_dir, f)
            for f in os.listdir(script_dir) if f.endswith('.csv')
        ]
        if possible_files:
            csv_file = possible_files[0]
            print(f"No CSV specified, using: {csv_file}")
        else:
            print("No CSV file specified and none found alongside this script.")
    ...
    try:
        ...
        print("[OK] Removed powerline interference")
        print("[OK] Applied frequency filtering")
        print("[OK] Removed baseline drift")
        print("[OK] Detected and interpolated artifacts")
        print("[OK] Normalized the signal")
        if 'improvement' in locals():
            print(f"[OK] Improved SNR by {improvement:.2f} dB")
        
        # Save processed data option (skipped when not running interactively,
        # e.g. under CI or a non-tty pipe, where input() would otherwise hang)
        if sys.stdin.isatty():
