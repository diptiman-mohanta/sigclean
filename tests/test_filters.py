...
class TestFilters(unittest.TestCase):
    ...
    def test_butterworth_lowpass(self):
        ...
        filtered = butterworth_filter(
            self.noisy_signal, self.fs, filter_type="lowpass", high_freq=30
        )
    ...
    def test_butterworth_highpass(self):
        ...
        filtered = butterworth_filter(
            self.drifted_signal, self.fs, filter_type="highpass", low_freq=1
        )
    ...
    def test_butterworth_bandpass(self):
        ...
        filtered = butterworth_filter(
            self.noisy_signal, self.fs, filter_type="bandpass", low_freq=5, high_freq=15
        )
    ...
    def test_notch_filter(self):
        ...
        filtered_high_q = notch_filter(
            self.noisy_signal, self.fs, freq=50.0, quality_factor=60
        )
    ...
    def test_baseline_removal_median(self):
        ...
        corrected = baseline_removal(
            self.drifted_signal, method="median", window_size=100
        )
    ...
    def test_baseline_removal_polynomial(self):
        ...
        corrected = baseline_removal(
            self.drifted_signal, method="polynomial", polynomial_order=2
        )
    ...
    def test_butterworth_over_nyquist_raises(self):
        """Frequencies at/above Nyquist must raise, never be silently clamped."""
        with self.assertRaises(ValueError):
            butterworth_filter(
                self.clean_signal, self.fs, filter_type="lowpass", high_freq=600
            )
        with self.assertRaises(ValueError):
            butterworth_filter(
                self.clean_signal,
    ...
    def test_butterworth_high_order_narrowband_is_stable(self):
        """Regression test: (b, a) transfer-function filtering previously
        produced all-NaN output for higher-order narrow-band designs; the
        SOS-based implementation must stay finite."""
        filtered = butterworth_filter(
            self.noisy_signal,
            self.fs,
            filter_type="bandpass",
    ...
    def test_notch_filter_over_nyquist_raises(self):
        """A notch frequency at/above Nyquist must raise, not silently pass through."""
        with self.assertRaises(ValueError):
            notch_filter(self.clean_signal, self.fs, freq=600.0)

    def test_moving_average_preserves_constant_signal(self):
        """Regression test: np.convolve(mode='same') implicitly zero-pads,
        biasing edge samples of a moving average toward zero. A constant
        signal must come back out unchanged, including at the edges."""
        data = np.full(200, 5.0)
        smoothed = moving_average_filter(data, 11)
        np.testing.assert_allclose(smoothed, 5.0)

    def test_baseline_removal_polynomial_long_signal_no_rank_warning(self):
        """Regression test: fitting np.polyfit on raw (unscaled) sample
        indices is ill-conditioned for long signals. Centering/scaling x
        should keep the fit well-conditioned even for a 1M-sample signal."""
        import warnings as warnings_module

        long_data = np.sin(2 * np.pi * 0.001 * np.arange(1_000_000)) + 1e-4 * np.arange(
            1_000_000
        )
        with warnings_module.catch_warnings():
            warnings_module.simplefilter("error")
            corrected = baseline_removal(
                long_data, method="polynomial", polynomial_order=5
            )
        self.assertFalse(np.any(np.isnan(corrected)))

    def test_artifact_removal_uses_robust_statistics(self):
        """A single large spike should not be masked by a mean/std estimate
        that the spike itself contaminates; MAD-based detection should
        catch it reliably."""
        data = np.random.RandomState(3).randn(5000)
        data[2500] += 15.0
        cleaned, mask = artifact_removal(data, fs=1000)
        self.assertTrue(mask[2500])
        self.assertLess(abs(cleaned[2500]), 5.0)
    ...
    def test_filter_output_types(self):
        ...
        result1 = butterworth_filter(
            self.clean_signal, self.fs, "lowpass", high_freq=50
        )
